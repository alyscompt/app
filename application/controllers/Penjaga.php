...
class Penjaga extends CI_Controller {
	...
	public function antrian_selanjutnya(){
		$tgl = date('dmY');
		$where =array('id_loket' => $this->session->userdata('loket'), 'tgl' => $tgl,'username '=> NULL);
		$antrian = $this->M_crud->get_min_id('transaksi','id_transaksi', $where);
		//$this->db->last_query();
		$data=array('id_loket' => $this->session->userdata('loket'), 'username' => $this->session->userdata('username'), 'tgl' => $tgl);
